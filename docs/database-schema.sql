-- AMS Trading Journal Web - Database Schema
-- Production-ready relational database design
...
CREATE TRIGGER update_calendar_stats_updated_at BEFORE UPDATE ON calendar_stats
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE POLICY "Traders can update their own trades" ON trades
    FOR UPDATE USING (auth.uid()::text = user_id::text);

CREATE POLICY "Traders can delete their own trades" ON trades
